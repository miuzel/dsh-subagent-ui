...
function backupsRoot() {
  return path.join(dshHome(), 'subagent-backups')
}
...
function backupSessionDirs(sessionId) {
  const found = findSessionDirs(sessionId)
  if (!found.length) return false
  const rootBackup = backupsRoot()
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-')
  const targetBackupDir = path.join(rootBackup, `${sessionId}_${timestamp}`)
  fs.mkdirSync(targetBackupDir, { recursive: true })

  for (const { slug, dir, variant } of found) {
    const dest = path.join(targetBackupDir, slug, variant)
...
async function deleteSingleSubagent(ctx, sessionId, parentSessionId, backup = false) {
  let backupDir = null
  if (backup) {
    backupDir = backupSessionDirs(sessionId)
  }
  ...
  return { sessionId, deleted: true, backupDir }
}
...
function registerHttpRoutes(webServer, ctx) {
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const { sessionId, parentSessionId, backup } = args
      if (!sessionId) {
        return sendJson(res, 400, { ok: false, error: 'missing-session-id' })
      }
      try {
        const result = await deleteSingleSubagent(ctx, sessionId, parentSessionId, Boolean(backup))
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        for (const item of items) {
          ...
          const resSingle = await deleteSingleSubagent(ctx, sid, undefined, Boolean(backup))
