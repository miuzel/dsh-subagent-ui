...
- 选择 N 小时以前的子代理。
- 批量归档 / 批量解除归档 / 一键全部归档。
- **批量删除（带备份）**：将选中的子代理先完整备份到 `~/.dsh/subagent-backups/` 目录，然后永久从磁盘、投影缓存、工作区索引和前端目录中彻底清除。
- **批量删除（不备份）**：直接永久清除选中的子代理。
- **一键删除全部**：一键彻底清理当前视图中的所有子代理（支持带备份/不备份）。

单项卡片在非批量模式下也提供 **🗑 删除** 按钮，点击可自由选择“带备份删除”或“不备份直接删除”。所有删除操作完全由人类用户在 UI 侧主动操作，子代理不会也不需要耗费时间去感知或调用工具。
